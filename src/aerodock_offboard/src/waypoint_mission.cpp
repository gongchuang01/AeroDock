...
private:
  ...
  double error_to_target() const {
    const auto &w = waypoints_[waypoint_index_];
  ...
  void transition(State next, const std::string &message) {
    state_ = next; state_start_ = now(); inside_since_ = rclcpp::Time(0, 0, get_clock()->get_clock_type());
  ...
  void tick() {
    ...
    if (state_ == State::NAVIGATING) {
      publish_target();
      if (!armed_ || !offboard_) {
        land("Flight state lost; landing");
        return;
      }
      if (position_valid_ && error_to_target() <= radius_) {
        ...
        if ((now() - inside_since_).seconds() >= hold_seconds_) {
          if (++waypoint_index_ >= waypoints_.size()) {
            ...
            inside_since_ = rclcpp::Time(0, 0, get_clock()->get_clock_type());
  ...
  rclcpp::Time mission_start_, state_start_, last_command_, inside_since_{0, 0, RCL_ROS_TIME};
  std::ofstream csv_;
